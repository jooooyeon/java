...
//		SqlSession session = sqlSessionFactory.openSession(); //파라미터를 안넣으면 오토커밋을 안쓰겠다 라는 뜻임
		...
		try {
			int cnt = session.insert("memberTest.insertMember", mv);	//파라미터가 있을 경우 뒤에 작성(여기서는 mv)
			if(cnt > 0) {
		...
		try {
			session = sqlSessionFactory.openSession(); // 오토커밋아님

			// 응답결과가 1개가 확실할 경우에는 selectOne()메서드를 이용한다.
			MemberVO mv4 = (MemberVO) session.selectOne("memberTest.getMember", "z5");

			System.out.println("I D : " + mv4.getMemId());
