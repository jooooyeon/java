...
public class T02FileTest {
	...
	/**
	 * 저장된 디렉토리(폴더)에 포함된 파일과 디렉토리 목록을 보여주기 위한 메서드
	 * dir 목록 보고싶은 폴더 객체
	 */
	...
	public static void displayFileList(File dir) {
		...
		//하위 디렉토리 정보를 저장할 List객체 생성(File배열의 인덱스 저장용)
		List<Integer> subDirList = new ArrayList<>();
		
		//날짜정보를 출력하기 위한 포멧 설정하기
		SimpleDateFormat sdf = new SimpleDateFormat("yyy-MM-dd a hh:mm");
		
		for(int i=0; i<files.length;i++) {
			String attr = ""; //파일의 속성 정보(읽기, 쓰기, 히든파일, 디렉토리 구분정보)
			String size =""; //파일용량
			...
			System.out.printf("%s %-5s %12s %s\n",
					...
					attr,size,files[i].getName()); //%-5s좌측정렬 , %5s 우측정렬
